...
import org.projectx.webservice.dao.util.ConditionQuery;
import org.projectx.webservice.dao.util.OrderBy;
import org.projectx.webservice.to.BaseTO;
import org.projectx.webservice.util.DateUtil;
import org.projectx.webservice.util.StringUtil;
import org.projectx.webservice.util.page.PageUtil;
...
public class BaseDAOImpl<M extends BaseTO, PK extends java.io.Serializable> implements BaseDAO<M, PK> {
...
//    private final String HQL_OPTIMIZE_NEXT_LIST_ALL;
    ...
    public BaseDAOImpl() {
        ...
        HQL_LIST_ALL = "from " + this.entityClass.getSimpleName() + " where isDeleted = 'N' order by " + pkName + " desc";
...
//        HQL_OPTIMIZE_NEXT_LIST_ALL = "from " + this.entityClass.getSimpleName() + " where " + pkName + " < ? order by " + pkName + " desc";
    ...
    public PK save(M model) {
    	...
    	if(StringUtil.isEmpty(userIdString)){
    		userIdString = Constants.ADMIN_USERID;
    	}
    	model.setCreateBy(userIdString);
    	model.setUpdateBy(userIdString);
    	model.setCreateDt(DateUtil.getDate());
    	model.setUpdateDt(DateUtil.getDate());
    	model.setIsDeleted(Constants.FLAG_N);
    ...
    public void update(M model) {
    	...
    	if(StringUtil.isEmpty(userName)){
    		userName = Constants.ADMIN_USERID;
    	}
        ...
        int versionNo = getVersionNo(id);
        model.setVersionNo(versionNo+1);
    ...
    public void delete(PK id) {
    	M model = get(id);
    	model.setIsDeleted(Constants.FLAG_Y);
    	update(model);
    }
    ...
    @Override
    public M get(PK id) {
    	String hqlString = "from " + this.entityClass.getSimpleName()+ " a where a.isDeleted = 'N' and a." + pkName + " = ?";
    	return this.unique(hqlString, new Object[]{id});
    }
    ...
    public <T> List<T> list(final String hql, final int pn, final int pageSize, final Object[] paramlist) {
        ...
        if (pn > -1 && pageSize > -1) {
            ...
            int start = PageUtil.getBeginIndex(pageSize, pn);
            if (start != 0) {
                query.setFirstResult(start);
            }
        }
